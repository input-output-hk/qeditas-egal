...
(* File: egalqeddoc.ml *)
(* Modified from Egal mgcheck.ml to create Qeditas signatures and documents. *)
...
(*** Code for building a Qeditas document or signature specification as the Egal file is processed ***)
let builddoc : Qsyntax.doc ref = ref []

let qeditassigna = ref false
let qeditasoutfile = ref "qeditasdoc";;
...
Axioms.initializeknowns indexknowns false;;
...
let evaluate_docitem_1 ditem =
  ...
  match ditem with
  | RequireDecl(x) ->
      if not !includingsigfile then builddoc := Qsyntax.DocSigna(Qhash.hexstring_hashval x)::!builddoc
  | Author(x,yl) ->
...
(*	  if not !includingsigfile then builddoc := Qsyntax.DocParam(Qsyntax.tm_hashroot (Qsyntax.Prim(xj)),ptp_to_qtp xi 0 xtp)::!builddoc; (* for Qeditas -- actually primitives should not be explicitly included in docs and signas. *) *)
	...
	with Not_found ->
	  begin
		...
		begin
		  try
		    let xqh = Hashtbl.find eqtmh xhv in
		    if not !includingsigfile then builddoc := Qsyntax.DocParam(xqh,tp_to_qtp 0 agtp)::!builddoc; paramtmh := xhv::!paramtmh; (* for Qeditas *)
		  with Not_found -> (Printf.printf "Do not know Qeditas hashval corresponding to tm with id %s\n" xhv; raise (Failure("")))
		end;
		begin
		  if !sqlout then
		    begin
  ...
  | DefDecl(x,None,b) ->
	...
	begin
	  ...
	  if not !includingsigfile then builddoc := Qsyntax.DocDef(ptp_to_qtp i 0 bgtp,ptm_to_qtm_lam_n i 0 bgtm)::!builddoc; deftmh := xhv::!deftmh; (* for Qeditas *)
	  begin
	    if !sqlout then
	      begin
		match !mainfilehash with
  ...
  | DefDecl(x,Some a,b) ->
	...
	begin
	  ...
	  let agtp = !tparclos atp in
	  let xhv = ptm_id (i,bgtm) sigtmof sigdelta in
	  if not !includingsigfile then builddoc := Qsyntax.DocDef(ptp_to_qtp i 0 agtp,ptm_to_qtm_lam_n i 0 bgtm)::!builddoc; deftmh := xhv::!deftmh; (* for Qeditas *)
	  begin
	    if !sqlout then
	      begin
		match !mainfilehash with
  ...
  | AxDecl(x,a) ->
	...
	raise (Failure(x ^ " has already been used."));
      ...
      if not !includingsigfile then builddoc := Qsyntax.DocKnown(ptm_to_qtm_all_n i 0 agtm)::!builddoc; (* for Qeditas *)
	...
	if !pfstate = [] then
	  begin
		...
		pushpolypf ((thmname,i),gpgtm);
	      ...
	      if not !includingsigfile then builddoc := Qsyntax.DocPfOf(ptm_to_qtm_all_n i 0 gpgtm,ppf_to_qpf_n i 0 dgpf)::!builddoc; (* for Qeditas *)
	    ...
	    with AdmittedPf ->
	      if not !includingsigfile then builddoc := Qsyntax.DocConj(ptm_to_qtm_all_n i 0 gpgtm)::!builddoc; (* for Qeditas *)
...
let _ =
  ...
  if i = 1 then
	...
	else if Sys.argv.(!j) = "-eqh" then (*** for Qeditas ***)
	  begin
	    if !j < i-3 then
	      begin
		incr j;
		eqtmhfile := Sys.argv.(!j);
		incr j;
		eqknhfile := Sys.argv.(!j);
		if Sys.file_exists !eqtmhfile then load_eqtmh();
		if Sys.file_exists !eqknhfile then load_eqknh()
	      end
	    else
	      raise (Failure("Expected -eqtmh <file> <file>"))
	  end
	else if Sys.argv.(!j) = "-qedsigout" then (*** for Qeditas ***)
	  begin
	    includingsigfile := false;
	    qeditassigna := true;
	    if !j < i-2 then
	      begin
		incr j;
		qeditasoutfile := Sys.argv.(!j)
	      end
	    else
	      raise (Failure("Expected -qedsigout <qeditasoutfilename>"))
	  end
	else if Sys.argv.(!j) = "-qeddocout" then (*** for Qeditas ***)
	  begin
	    includingsigfile := false;
	    qeditassigna := false;
	    if !j < i-2 then
		...
		qeditasoutfile := Sys.argv.(!j)
	      ...
	      raise (Failure("Expected -qeddocout <qeditasoutfilename>"))
	  end
...
(*** Finally, output the document or signature specification serialized for Qeditas. ***)
let docitem_signaitem d =
  match d with
  | Qsyntax.DocSigna(h) -> Qsyntax.SignaSigna(h)
  | Qsyntax.DocParam(h,a) -> Qsyntax.SignaParam(h,a)
  | Qsyntax.DocDef(a,m) -> Qsyntax.SignaDef(a,m)
  | Qsyntax.DocKnown(p) -> Qsyntax.SignaKnown(p)
  | Qsyntax.DocConj(p) -> raise (Failure("Signature should not contain conjectures."))
  | Qsyntax.DocPfOf(p,d) -> raise (Failure("Signature should not contain proofs."));;

let ch = open_out_bin !qeditasoutfile;;
if !qeditassigna then
  let s = List.map docitem_signaitem !builddoc in
  let c = Qsyntax.seo_signaspec Qser.seoc s (ch,None) in
  Qser.seocf c
else
  let c = Qsyntax.seo_doc Qser.seoc !builddoc (ch,None) in
  Qser.seocf c;;
close_out ch;;
