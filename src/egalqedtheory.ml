...
(* File: egalqedtheory.ml *)
(* Modified from Egal mgcheck.ml to create Qeditas theories. *)
(* It does not require axioms to be listed in axioms.ml to be accepted. *)
...
(*** Code to build a Qeditas theory specification. ***)
let buildtheoryspec : Qsyntax.theoryspec ref = ref []

let nextprim = ref 0

let qeditasoutfile = ref "qeditastheoryspec"
...
let evaluate_docitem_1 ditem =
  ...
  match ditem with
  | RequireDecl(x) -> raise (Failure("A theory cannot require a signature."))
  | Author(x,yl) ->
  ...
  | ParamDecl(x,a) ->
	...
	try
	  ...
	  if not (!nextprim = xj) then raise (Failure(x ^ " cannot be declared here. Primitives must be declared in order.")); (* for Qeditas *)
	  incr nextprim; (* for Qeditas *)
	  buildtheoryspec := Qsyntax.ThyPrim(ptp_to_qtp xi 0 xtp)::!buildtheoryspec; (* for Qeditas *)
  ...
  | DefDecl(x,None,b) ->
	...
	begin
	  ...
	  buildtheoryspec := Qsyntax.ThyDef(ptp_to_qtp i 0 bgtp,ptm_to_qtm_lam_n i 0 bgtm)::!buildtheoryspec; deftmh := xhv::!deftmh; (* for Qeditas *)
	  begin
	    if !sqlout then
	      begin
		match !mainfilehash with
  ...
  | DefDecl(x,Some a,b) ->
	...
	begin
	  ...
	  let agtp = !tparclos atp in
	  let xhv = ptm_id (i,bgtm) sigtmof sigdelta in
	  buildtheoryspec := Qsyntax.ThyDef(ptp_to_qtp i 0 agtp,ptm_to_qtm_lam_n i 0 bgtm)::!buildtheoryspec; deftmh := xhv::!deftmh; (* for Qeditas *)
	  begin
	    if !sqlout then
	      begin
		match !mainfilehash with
  ...
  | AxDecl(x,a) ->
	...
	raise (Failure(x ^ " has already been used."));
      ...
      buildtheoryspec := Qsyntax.ThyAxiom(ptm_to_qtm_all_n i 0 agtm)::!buildtheoryspec; (* for Qeditas *)
...
let _ =
  ...
  if i = 1 then
	...
	else if Sys.argv.(!j) = "-qedts" then (*** for Qeditas ***)
	  begin
	    includingsigfile := false;
	    if !j < i-2 then
	      begin
		...
		qeditasoutfile := Sys.argv.(!j)
	      ...
	      raise (Failure("Expected -qedts <qeditasoutfilename>"))
	  end
...
(*** Finally, output the theory specification serialized for Qeditas. ***)
let ch = open_out_bin !qeditasoutfile;;
let c = Qsyntax.seo_theoryspec Qser.seoc !buildtheoryspec (ch,None);;
Qser.seocf c;;
close_out ch
